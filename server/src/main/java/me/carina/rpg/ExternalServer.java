    ...
    public void open(int port){
        server = new WebSocketServer(new InetSocketAddress(port)) {
            ...
            public void onClose(WebSocket conn, int code, String reason, boolean remote) {
                if (ExternalServer.this.removeConnection(ExternalServer.this.getConnections().get(conn))) getLogger().error("Client disconnected " + conn.getProtocol().toString());
                else getLogger().error("Received disconnect message from unknown connection "+conn.getProtocol().toString());
            }

            @Override
            public void onMessage(WebSocket conn, String message) {
                recieve(getSerializer().deserialize(message), ExternalServer.this.getConnections().get(conn));
            }
