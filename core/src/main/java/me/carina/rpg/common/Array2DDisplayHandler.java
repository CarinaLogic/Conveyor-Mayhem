...
public abstract class Array2DDisplayHandler<T extends Feature> {
    Group group;
    Consumer<T> addFunc;
    Consumer<Actor> removeFunc;
    public Array2DDisplayHandler(Group group, Consumer<T> addFunc){
        this(group,addFunc, Actor::remove);
    }
    public Array2DDisplayHandler(Group group, Consumer<T> addFunc, Consumer<Actor> removeFunc){
        this.group = group;
        this.addFunc = addFunc;
        this.removeFunc = removeFunc;
    }
    public abstract Array2DFeature<T> getIterable();
    public void tick() {
        Array2DFeature<T> iterable;
        try {
        ...
        for (Array2D.Array2DEntry<T> entry : getIterable()) {
            boolean success = false;
            if (entry.value != null) {
                for (Actor display : Game.getClient().getDisplays().getAll(entry.value)) {
